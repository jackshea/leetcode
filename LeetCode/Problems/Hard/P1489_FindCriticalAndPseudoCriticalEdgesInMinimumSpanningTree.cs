...
namespace LeetCode.Problems.Hard
{
    ...
    public class P1489_FindCriticalAndPseudoCriticalEdgesInMinimumSpanningTree
    {
        public IList<IList<int>> FindCriticalAndPseudoCriticalEdges(int n, int[][] edges)
        {
            ...
            List<int> keyEdges = new List<int>();
            List<int> notKeyEdges = new List<int>();
            List<IList<int>> ans = new List<IList<int>> { keyEdges, notKeyEdges };
            for (var i = 0; i < edgeList.Count; i++)
            {
                ...
                {
                    var edge = edgeList[j];
                    if (i != j && uf.Union(edge.P1, edge.P2))
                    {
                        cost += edge.Weight;
                    }
                }
                ...
                if (cost == minCost)
                {
                    notKeyEdges.Add(edgeList[i].Index);
                }
            }
