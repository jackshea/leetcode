...
public class P0417_PacificAtlanticWaterFlow
{
    ...
    public IList<IList<int>> PacificAtlantic(int[][] matrix)
    {
        ...
        // 从四条边界开始DFS
        for (var i = 0; i < matrix.Length; i++) DFS(matrix, 0, i, 0, 1);

        for (var i = 0; i < matrix[0].Length; i++) DFS(matrix, 0, 0, i, 1);

        for (var i = 0; i < matrix.Length; i++) DFS(matrix, 0, i, matrix[0].Length - 1, 2);

        for (var i = 0; i < matrix[0].Length; i++) DFS(matrix, 0, matrix.Length - 1, i, 2);

        // 从 mark 中得到答案
        for (var i = 0; i < mark.GetLength(0); i++)
        for (var j = 0; j < mark.GetLength(1); j++)
            // 代表同时被太平洋和大西洋标记
            if (mark[i, j] == 3)
