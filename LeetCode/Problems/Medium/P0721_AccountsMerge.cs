...
namespace LeetCode.Problems.Medium
{
    ...
    public class P0721_AccountsMerge
    {
        ...
        {
            Dictionary<string, int> email2Index = new Dictionary<string, int>();
            Dictionary<string, string> email2Name = new Dictionary<string, string>();
            int emailIndex = 0;
            ...
            foreach (var account in accounts)
            {
                int firstIndex = email2Index[account[1]];
                for (var j = 2; j < account.Count; j++)
                {
                    unionFind.Union(firstIndex, email2Index[account[j]]);
                }
            }
            Dictionary<int, List<string>> index2Emails = new Dictionary<int, List<string>>();
            foreach (var kvp in email2Index)
            {
                int index = unionFind.Find(kvp.Value);
            ...
            {
                List<string> row = new List<string> { email2Name[kvp.Value[0]] };
