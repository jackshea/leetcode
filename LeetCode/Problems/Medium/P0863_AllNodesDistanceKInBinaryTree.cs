...
public class P0863_AllNodesDistanceKInBinaryTree
{
    ...
    private readonly Dictionary<TreeNode, TreeNode> parent = new();
    private readonly HashSet<TreeNode> visited = new();
    ...
    private void DFS(TreeNode node, int depth, int k)
    {
        ...
        if (parent.TryGetValue(node, out var p)) DFS(p, depth + 1, k);
    }
}