...
public class P0692_TopKFrequentWords
{
    public IList<string> TopKFrequent(string[] words, int k)
    {
        ...
        var pq = new PriorityQueue<string>(k, Comparer<string>.Create((a, b) =>
        {
            if (freq[a] != freq[b]) return -freq[a].CompareTo(freq[b]);
        ...
        var ans = new List<string>(k);
        for (var i = 0; i < k; i++) ans.Add(pq.Pop());
        ans.Reverse(0, k);
        return ans;
    }
