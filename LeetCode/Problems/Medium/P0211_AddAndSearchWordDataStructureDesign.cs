...
public class WordDictionary
{
    ...
    public void AddWord(string word)
    {
        ...
        for (var i = 0; i < word.Length; i++)
        {
            var c = word[i];
            var ci = c - 'a';
            if (node.Nexts[ci] == null) node.Nexts[ci] = new TrieNode();
    ...
    public bool Search(TrieNode node, string word)
    {
        ...
        for (var i = 0; i < word.Length; i++)
        {
            ...
            if (c == '.')
            {
                ...
                for (var j = 0; j < 26; j++)
                {
                    if (node.Nexts[j] == null) continue;

                    if (Search(node.Nexts[j], substring)) return true;
                }
            ...
            var ci = c - 'a';
            if (node.Nexts[ci] == null) return false;
