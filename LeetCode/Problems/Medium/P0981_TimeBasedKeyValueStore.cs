...
{
    private readonly Dictionary<string, List<Tuple<int, string>>> dic;
    ...
    {
        if (!dic.TryGetValue(key, out var list) || list.Count == 0) return string.Empty;

        if (list[0].Item1 > timestamp) return string.Empty;
        ...
        if (left >= list.Count || list[left].Item1 != timestamp) return list[left - 1].Item2;
