...
namespace LeetCode.Problems.Medium
{
    ...
    {
        private int[][] direction = { new[] { 0, -1 }, new[] { -1, 0 }, new[] { 1, 0 }, new[] { 0, 1 } };
        public int MinimumEffortPath(int[][] heights)
        {
            PriorityQueue<int[]> pq = new PriorityQueue<int[]>(16, (a, b) => -a[2].CompareTo(b[2]));
            ...
            while (pq.Count != 0)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        int nIdx = nx * n + ny;
                        int cost = Math.Abs(heights[x][y] - heights[nx][ny]);
                        int maxCost = Math.Max(cost, d);
