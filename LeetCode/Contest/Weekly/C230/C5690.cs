...
public class C5690
{
    ...
    {
        for (var i = 0; i < baseCosts.Length; i++) DFS(toppingCosts, 0, 0, baseCosts[i], target);
    ...
    {
        if (closet == target) return;

        if (Math.Abs(closet - target) == Math.Abs(sum - target))
            closet = Math.Min(closet, sum);
        else if (Math.Abs(closet - target) > Math.Abs(sum - target)) closet = sum;

        if (toppingIdx >= toppingCosts.Length || sum >= target) return;
